function [doppler_fft, range_fft, range_angle] = fft_features(Receiver_1, Receiver_2, Receiver_3, Receiver_4, numADCSamples, sampling_rate, freq_slope, frame, m_chirps, angle_padding_count)
    ...
    range_axis = (0:(numADCSamples/2 - 1)) * range_resolution; % Range Axis (m)
    ...
    for cnt = 1:8
            ...
            % Remove signals with no variation (static signals)
            range_magnitude = abs(range_fft);
            threshold = 1e+3;  % Adjust threshold based on signal characteristics
            range_fft(range_magnitude < threshold) = 0;
            
            % 1D FFT along the Doppler dimension
            doppler_fft = fft(range_fft, [], 2);
            doppler_fft = fftshift(doppler_fft, 2);
            
            % Suppress low magnitude values
            doppler_fft(abs(doppler_fft) < 1e+4) = 0;
            ...
            if cnt == 1       
                figure(1)
                subplot(1, 2, 1);
                imagesc(velocity_axis, range_axis, 20 * log10(abs(doppler_fft(1:numADCSamples/2, :))));
    ...
    imagesc(x_axis_img, range_axis, abs(range_angle(1:numADCSamples/2, :)));
